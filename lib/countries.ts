...
const ALL_COUNTRIES: { name: string; dialCode: string }[] = [
...
// Sri Lanka pinned to the top (as the default/most-used option here), then
// every other country alphabetically.
export const COUNTRY_CODES: { name: string; dialCode: string }[] = [
  ALL_COUNTRIES.find((c) => c.name === "Sri Lanka")!,
  ...ALL_COUNTRIES.filter((c) => c.name !== "Sri Lanka"),
];

// Splits a stored phone value (with or without a dial code prefix) into a
// {country, local} pair for pre-filling the country/number fields when
// editing. Falls back to the default country with the raw value as the
// local part if no known dial code prefixes it (covers numbers saved
// before this feature existed).
export function splitPhone(phone: string | null | undefined): { country: string; local: string } {
  const trimmed = (phone ?? "").trim();
  if (!trimmed) return { country: DEFAULT_COUNTRY_CODE, local: "" };

  const sorted = [...ALL_COUNTRIES].sort((a, b) => b.dialCode.length - a.dialCode.length);
  for (const c of sorted) {
    if (trimmed.startsWith(c.dialCode)) {
      return { country: c.dialCode, local: trimmed.slice(c.dialCode.length).trim() };
    }
  }
  return { country: DEFAULT_COUNTRY_CODE, local: trimmed };
}

export function combinePhone(country: string, local: string): string {
